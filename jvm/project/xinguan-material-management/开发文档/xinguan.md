...
为项目起一个名字并选择包管理器，我习惯使用npm。勾选git仓库初始化，将代码交予git管理。（前提是你本机已经安装并配置了git）

![1597740333312](xinguan/1597740333312.png)

我们第一次创建项目，选择“手动”创建。即：根据自己的需要选择插件及相关的配置项。

![img](xinguan/45828c8db3ebadacd0bfe1af815e29f5_1458x772.png)

![img](xinguan/5e1125ebbc2f1eca4d514f877cd05be6_1444x356.png)

根据自己的需要选择功能，上图中我选择了babel、vue router、vuex 、CSS预处理器，使用单独的配置文件。如果你不知道这些东西是什么，应该先去学习vue基础，[vue官网](https://cn.vuejs.org/)。本系列课程着重于实战，基础知识不做讲解。
...
![img](xinguan/672953675045736038690f3cc7689291_1618x517.png)

- 提示选择vue前端路由的模式，我们使用history模式
- 提示选择CSS预处理器，我习惯使用Sass（node-sass）。你可以根据自己的情况进行选择。

![1597741274458](xinguan/1597741274458.png)

在所有的选择配置完成之后，会提示我们是否将我们的“选择”保存为一个“预设”，下次我们再创建类似项目的时候就直接选择该“预设”就可以了，不需要一项一项的勾选。我这里就先不保存了，“创建项目，不保存预设”。
...
![1607265627143](xinguan/1607265627143.png)

如遇到该错误，先使用如下命令。然后删除文件夹，重新安装。
...
使用vue/cli创建项目之后，文件及目录结构如下。但是在实际的开发中，我们需要对文件及目录进行细化调整，以适应和方便我们的开发。

![img](xinguan/9580adb0405dde2b9607f39f2001beca_721x769.png)

**vue.config.js**

我们所有的开发过程代码都放在src的目录下面，所以在很多位置我们都希望引用该文件夹下面的模块。为了方便引用该文件夹下面的模块，我们为该文件夹起一个别名叫做'@'。
这个别名需要我们自己来配置，配置文件是vue.config.js，该配置文件通常用于修改vue项目的基础配置和webpack基础配置。该配置文件默认是不被创建的，需要我们手动在项目的**根目录**下面创建。
...
当我们希望引用Home组件的时候，可以使用‘‘@/views/Home.vue’’。当然vue.config.js不只可以做文件夹别名的配置还可以做很多其他的配置，我们用到的时候再讲。

**assets文件夹细分**

assets文件夹下面放一些图片、字体等静态的资源文件。我们可以做一下文件夹的细分处理，比如：新建一个img文件夹存放图片，新建一个font文件夹存放
![img](xinguan/5973e2f0770a5ebc16b0a0933a90b5ec_238x114.png)

完成基础调整之后，文件夹目录及文件结构如下：当然随着项目的深入，我们还会新增一些其他的文件夹。
